...
6. **Clipboard privacy markers are desktop-only.** Browser (`navigator.clipboard`)
   ...
   web/mobile manual "Add Current Clipboard" cannot honor them. _Decision:_
...
- **No global aggregate cap on live WebSocket connections.** Per-user concurrent
  ...
  cadences are fixed constants in `ws.rs`, not config. _Fix if it matters:_ a
...
- **No whole-request HTTP deadline on the native client.**
  ...
  operation rather than freezing the React Native UI. _Fix if it matters:_ a short
