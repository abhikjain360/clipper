...
- [ ] `src/engine.rs` (550 lines). The expansion.
  - `rule_spans`: builds the DTSTART/RRULE text for the `rrule` crate on
    wall-clock time, then resolves every candidate through `time.rs`. Ask:
    where does each of the two limits (65,535 scanned, 10,000 in window)
    bite, and does exceeding either error rather than truncate?
...
- [ ] `crates/server/src/storage_quota.rs`. `revision_cost_bytes` is the one
      ...
      `try_reserve_user_storage` and `release_user_storage` are the only counter
      writers. `revision_usage_by_user` gives bytes; `object_usage_by_user` gives
...
- [ ] `crates/client/src/engine.rs`, acceptance and sync functions:
  ...
  - `check_revision_advance`, `download_file_bytes`: a fetched head goes
    through the same anchor rules as a listed one and is retained.
...
- [ ] `crates/schedule/src/ingest.rs`: `parse_ics`,
      ...
      `validate_rrule_counts`. Limits: 8 MiB feed, 50,000 components, 500,000
...
| S2  | Reserving zero bytes and zero objects still ran the quota predicate, so a user above a lowered limit could not tombstone, so could not purge.                                                          | `storage_quota.rs` `try_reserve_user_storage` | Fixed in "Reject clipboard revisions early and harden server cleanup and quota paths" |
...
| #   | Finding                                                                                                                                      | Where                                       | Status                                                                                        |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------- |
| I1  | The calcard parser rewrote `INTERVAL=0`, `COUNT=0` and negative values into a valid daily cadence before the converter saw them.             | `ingest.rs` `rrule_text`                    | Fixed in "Reject rewritten import rules, cap overrides, validate day ordinals on deserialize" |
