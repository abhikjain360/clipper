...
| F3  | TZIDs were parsed as IANA names only, so Outlook and Office 365 feeds (`W. Europe Standard Time`) were rejected wholesale.                                                                                                | `ingest.rs` `feed_time_from_partial`                                      | Fixed in "Accept old, Outlook and BOM-prefixed calendar feeds and close the escaped RRULE bypass"; an `Etc/` zone guessed from a UTC offset is refused since "Refuse RRULE lines with quoted or escaped names and zones guessed from an offset"                             |
...
| F5  | Backslash escapes (`INTER\VAL=65536`, `RRUL\E:`, `\"` inside a quoted parameter) hid numeric clauses from the pre-check, so calcard still narrowed them. R6 and T8 were incomplete.                                       | `ingest.rs` `value_separator`, `validate_rrule_numbers`                   | Fixed in the same commit and completed in "Refuse RRULE lines with quoted or escaped names and zones guessed from an offset" (quoted names, backslash-tab)                                                                                                                  |
...
| F11 | A listed item whose download failed was not marked seen, so the sweep dropped the held copy; a schedule item and its alarm vanished until the next pass. The clipboard path had the same gap on `main`.                   | `engine.rs` `keep_held_revision`, `snapshot_schedule`                     | Fixed in "Harden client sync, timers and the WebSocket found in the fourth review"; its skip of already-held schedule items was removed in "Keep same-revision tombstone conflicts visible and download every listed schedule item"                                         |
...
| F14 | A write the server accepted but that another device's newer revision had already overtaken locally was reported to the user as an error.                                                                                  | `engine.rs` `publish_accepted_write`; `local_store.rs` `holds_newer_than` | Fixed in "Harden client sync, timers and the WebSocket found in the fourth review"; a same-revision tombstone counts as a conflict since "Keep same-revision tombstone conflicts visible and download every listed schedule item"                                           |
...
| F21 | Only the Logout button cancelled Android alarms, so a session the engine ended itself (device removed elsewhere) kept ringing the old account's alarms.                                                                   | `mobile/src/App.tsx`                                                      | Fixed in "Cancel Android alarms when the session ends without the Logout button" and "Clear Android alarms when a cold-start resume is refused"                                                                                                                             |
...
| F23 | The week grid and the next-occurrence sort re-ran their expansions on every state publish.                                                                                                                                | `web/src/SchedulePanel.tsx`                                               | The sort is fixed in "Move a monthly rule with its block and re-expand the week only on content changes"; the week grid change was reverted in "Re-expand the week on every state change again" because `AppState` has no key for recordings and overrides; see the backlog |

A second review of these fixes by GPT-6 Astra verified seventeen rows and
found gaps in six: F3 accepted an `Etc/` zone calcard guessed from a
"(UTC+05:30)" display name, F5 missed quoted names and backslash-tab, F11's
download skip trusted a persisted head whose record was not loaded, F14
treated a same-revision tombstone as newer, F21 missed a resume refused at
cold start, and F23's keys missed revisions, recordings and overrides. The
Status column above names the follow-up commits.
